...
exports.getAllCartProducts = async (req) => {
  try {
    if (req.isVendor) {
      ...
      const pageSize = 10;
      // const skip = pageNumber === 1 ? 0 : parseInt((pageNumber - 1) * pageSize);
      let result = await makeMongoDbServiceCart.getDocumentByQueryPopulate(
        {},
        null,
      ...
      result = result.filter((cart) => {
        const products = cart.cartItems.filter((product) => {
          ...
          return (
            ...
            product.product.vendor.toString() == req.vendor._id.toString()
          );
        });
      ...
      return response(
        true,
        null,
        "Only vendors can access this API",
        null,
        403
      );
    }
